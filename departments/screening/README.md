...
> 本目录是**筛分科室**的独立产品项目，与热工（`departments/thermal/`）、搅拌（`departments/mixing/`）完全隔离，可在本科室目录内**独立启动、独立开发、独立部署**。

## 启动方式

```bash
cd departments/screening
python app.py        # 端口 5013
```

- 门户（5010）通过 `portal/config/portal_config.json` 中 `screening.route` 跳转至此。
...
├── app.py                    # ★ 独立服务入口（占位首页 + 静态托管，端口 5013）
├── static/
│   ├── js/
...
1. **自包含原则**：筛分科室的所有资源（含后端代码）放在本目录内，禁止引用其他科室（`departments/thermal/`、`departments/mixing/`）的文件。
