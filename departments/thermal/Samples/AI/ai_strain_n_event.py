...
with artdaq.Task() as task:
    task.ai_channels.add_ai_straingage_chan("Dev6/ai0")
    task.timing.cfg_samp_clk_timing(rate=10000.0, sample_mode=AcquisitionType.CONTINUOUS)
    task.calibration.perform_bridge_offset_nulling_cal()
    task.calibration.perform_strain_shunt_cal(channel="", shunt_resistor_val=100000, shunt_resistor_location=0, skip_unsupported_channels=0)
